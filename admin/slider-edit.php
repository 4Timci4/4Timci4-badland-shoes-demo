                ...
                <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                    ...
                    <div class="p-6">
                        <div id="slider-preview" class="relative h-64 rounded-xl overflow-hidden"
                            style="background-color: <?= htmlspecialchars($_POST['bg_color'] ?? '#f0f0f0') ?>;">
                            <div id="preview-bg-image"
                                class="absolute inset-0 bg-cover bg-center <?= empty($_POST['image_url']) ? 'hidden' : '' ?>"
                                style="background-image: url('<?= htmlspecialchars($_POST['image_url'] ?? '') ?>');">
                            </div>
                            <div class="absolute inset-0 bg-black opacity-40 <?= empty($_POST['image_url']) ? 'hidden' : '' ?>"
                                id="preview-overlay"></div>
                            <div class="relative z-10 h-full flex items-center justify-center text-center text-white">
                                <div class="max-w-md px-4">
                                    <h2 id="preview-title" class="text-2xl md:text-3xl font-bold mb-3">
                                        <?= htmlspecialchars($_POST['title'] ?? 'Slider Başlığı') ?></h2>
                                    <p id="preview-description" class="text-sm md:text-base mb-4">
                                        <?= htmlspecialchars($_POST['description'] ?? 'Slider açıklaması burada görünecek') ?>
                                    </p>
                    ...
                    <div class="p-6">
                        <div class="space-y-3">
                            <label class="flex items-center">
                                <input type="checkbox" name="is_active" value="1" <?= ($_POST['is_active'] ?? '0') === '1' ? 'checked' : '' ?>
