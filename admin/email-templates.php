            ...
            <?php if ($edit_mode && $edit_template): ?>
                <form method="POST" class="bg-white rounded-2xl shadow-lg border border-gray-100">
                    ...
                    <div class="p-6 border-b">
                        <h3 class="text-lg font-bold">"<?= htmlspecialchars($edit_template['description']) ?>" Şablonunu
                            Düzenle</h3>
                    </div>
                        ...
                        <div id="content-tab" class="tab-content active space-y-6">
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="text" name="subject"
                                        value="<?= htmlspecialchars($edit_template['subject']) ?>"
                            ...
                            <div>
                                <div class="flex items-center justify-between mb-3">
                                    ...
                                    <div class="flex space-x-2">
                                        <button type="button" id="insert-variables-btn"
                                            class="text-sm text-primary-600 hover:text-primary-700 font-medium">
                                            <i class="fas fa-plus-circle mr-1"></i>Değişken Ekle
                                        </button>
                                        <button type="button" id="format-html-btn"
                                ...
                                <div class="border border-gray-300 rounded-lg overflow-hidden">
                                    <textarea id="html-editor" name="body_html"
                                        class="w-full"><?= htmlspecialchars($edit_template['body_html']) ?></textarea>
                                </div>
                            </div>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-3">
                                    ...
                                    <span class="text-xs text-gray-500 ml-2">(E-posta istemcileri HTML desteklemediğinde
                                        kullanılır)</span>
                                </label>
                                ...
                                <p class="text-xs text-gray-500 mt-1">Bu alan, HTML içerikten otomatik olarak oluşturulur.
                                </p>
                            </div>
                        </div>
                                ...
                                <div class="bg-white rounded-lg shadow-lg max-w-2xl mx-auto">
                                    ...
                                    <div class="p-6">
                                        <div class="mb-4 pb-4 border-b">
                                            <h3 class="font-semibold text-lg text-gray-800">Konu: <span
                                                    id="preview-subject"></span></h3>
                                        </div>
                        ...
                        <div id="variables-tab" class="tab-content hidden">
                            <div class="space-y-4">
                                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                                    <h4 class="font-semibold text-blue-900 mb-3"><i class="fas fa-user mr-2"></i>Kullanıcı
                                        Değişkenleri</h4>
                                    <p class="text-sm text-blue-800 mb-4">Bu değişkenler, e-postanın gönderildiği
                                        kullanıcıya göre dinamik olarak değişir.</p>
                                    <div class="space-y-2">
                                        <button type="button" class="variable-btn"
                                            data-variable="{{fullName}}"><code>{{fullName}}</code> - Kullanıcının tam
                                            adı</button>
                                    </div>
                                </div>
                                <div class="bg-green-50 border border-green-200 rounded-lg p-4">
                                    <h4 class="font-semibold text-green-900 mb-3"><i class="fas fa-globe mr-2"></i>Site
                                        Değişkenleri</h4>
                                    <p class="text-sm text-green-800 mb-4">Bu değişkenler, sitenizin genel ayarlarından
                                        alınır.</p>
                                    <div class="space-y-2">
                                        <button type="button" class="variable-btn"
                                            data-variable="{{site_url}}"><code>{{site_url}}</code> - Site ana
                                            URL'si</button>
                                        <button type="button" class="variable-btn"
                                            data-variable="{{current_year}}"><code>{{current_year}}</code> - Güncel
                                            yıl</button>
                                        <button type="button" class="variable-btn"
                                            data-variable="{{contact_url}}"><code>{{contact_url}}</code> - İletişim sayfası
                                            URL'si</button>
                                    </div>
                                </div>
                            </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        window.updatePreview = function () {
            ...
            let previewHtml = editorContent.replace(/{{fullName}}/g, 'Ahmet Yılmaz')
                .replace(/{{site_url}}/g, '<?= (APP_ENV === 'development' ? 'http://localhost:3000' : 'https://badlandshoes.com.tr') ?>')
