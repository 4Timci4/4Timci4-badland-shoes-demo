...
class SupabaseClient {
     ...
     */
    public function request($endpoint, $method = 'GET', $data = null, $useCache = null) {
    ...
    public function executeRawSql($sql, $params = []) {
        ...
        return $this->request('/rest/v1/rpc/execute_sql', 'POST', [
            'query' => $preparedSql
        ], false); // Önbellek kullanma
    }
}

/**
 * Singleton SupabaseClient örneği döndürür
 * 
...
function supabase() {
    ...
    if ($instance === null) {
        ...
        try {
            $instance = new SupabaseClient(SUPABASE_URL, SUPABASE_KEY, $options);
        } catch (Exception $e) {
            error_log('Supabase bağlantı hatası: ' . $e->getMessage());
            // Kritik bir hata, uygulama durmalı
...
/**
 * Eski Supabase istek fonksiyonu - Geriye uyumluluk için
 *
 * @param string $endpoint API endpoint'i
 * @param string $method HTTP metodu (GET, POST, PATCH, DELETE)
...
function supabase_request($endpoint, $method = 'GET', $data = null) {
    try {
        return supabase()->request($endpoint, $method, $data);
    } catch (Exception $e) {
        error_log('Supabase istek hatası: ' . $e->getMessage());
        return [];
    }
}
